...
class noticeAPI extends SlimvcController
{
    ...
    function newNotice()
    {
        try {
            ...
            $notice_content=trim($json['notice_content']);
            $notice_groups=$json['notice_groups'];
            $notice_end_time=intval($json['notice_end_time']);
            if(empty($notice_title) || empty($notice_content) ||count($notice_groups)==0 ||$notice_end_time<time() )
            ...
            unset($one);
            if(!($notice_id=$notice_model->newNotice($user_id,$notice_title,$notice_content,time(),$notice_end_time)))
